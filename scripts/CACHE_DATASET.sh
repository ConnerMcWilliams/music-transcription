...
mkdir -p $LIST_DIR
python $DATASET_SCRIPTS/make_list_maestro.py -i $MAESTRO_DIR/maestro-v3.0.0/maestro-v3.0.0.csv -d_list $LIST_DIR

# 3. rename the files
mkdir -p $MIDI_DIR
mkdir -p $WAV_DIR
python $DATASET_SCRIPTS/rename_maestro.py -d_i $MAESTRO_DIR/maestro-v3.0.0 -d_o $MAESTRO_DIR -d_list $LIST_DIR

# 4. convert wav to log-mel spectrogram
mkdir -p $FEATURE_DIR
python $DATASET_SCRIPTS/conv_wav2fe.py -d_list $LIST_DIR -d_wav $WAV_DIR -d_feature $FEATURE_DIR -config $CONFIG_FILE

# 5. convert midi to note
mkdir -p $NOTE_DIR
python $DATASET_SCRIPTS/conv_midi2note.py -d_list $LIST_DIR -d_midi $MIDI_DIR -d_note $NOTE_DIR -config $CONFIG_FILE

# 6. convert note to label
mkdir -p $LABEL_DIR
python $DATASET_SCRIPTS/conv_note2label.py -d_list $LIST_DIR -d_note $NOTE_DIR -d_label $LABEL_DIR -config $CONFIG_FILE
...
python $DATASET_SCRIPTS/conv_note2ref.py -f_list $LIST_DIR/valid.list \
    -d_note $NOTE_DIR -d_ref $REFERENCE_DIR